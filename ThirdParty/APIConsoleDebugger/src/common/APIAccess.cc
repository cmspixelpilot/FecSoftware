...
int readFile ( FecAccess *fecAccess,
  ...
  while (fgets (strFrame, MAXCAR, fichier) != NULL) {
	...
	case 'n': 
	  ...
	  break ;
        case 'H':case 'h':
          if (strncasecmp(str, "hardreset", 4) == 0){
            str=NULL;
	    fecRegisterAccess = true ;
	    frame[0] = 0 ;
          }
      ...
      try {
        ...
        if (fecRegisterAccess) {
	      ...
	      std::cout << "write 0x" << std::hex << (int)frame[1] << " in CR1" << std::endl ;
            ...
            break;
          case 'h':case 'H': //hard reset
	      std::cout << "FEC hard reset" << std::endl ;
              fecDevice->fecHardReset () ;
              break ; 
          }
          fecRegisterAccess = false ;
	...
	for (Sgi::hash_map<keyType, bool *>::iterator p=trackerModule.begin();p!=trackerModule.end();p++) {
	  ...
	  if (module != NULL) {
	    ...
	    if (apv6F || apv4F || pllF || muxF || aohF) {
	      std::cout << "Found a module on FEC " << std::dec << (int) getFecKey(indexF) << " Ring " << (int) getRingKey(indexF) << " CCU 0x" << std::hex << (int) getCcuKey(indexF) << " channel 0x" << std::hex << (int)getChannelKey(indexF) << std::endl ;
