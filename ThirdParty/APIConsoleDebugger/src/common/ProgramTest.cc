...
int main ( int argc, char *argv[] ) {
  ...
  for (int i = cnt ; i < argc ; i ++) {
	...
	status = false ;
    ...
    else if (strcasecmp (argv[i],"-utca") == 0 || strcasecmp (argv[i],"-fechardwareid") == 0) {
            status = true; // no error during this setting
            i++;
    }
      ...
      if ( strncmp(argv[i], "-pci", strlen("-pci")) && strncmp(argv[i], "-usb", strlen("-usb")) && strncmp(argv[i], "-vmecaenpci", strlen("-vmecaenpci")) && strncmp(argv[i], "-vmesbs", strlen("-vmesbs")) && strncmp(argv[i], "-vmecaenusb", strlen("-vmecaenusb")) && strncmp(argv[i], "-utca", strlen("-utca"))) {
