...
//Edit G Auzinger
FecAccess::FecAccess ( const std::string& strFilename, const std::string& strBoardId, const std::string& strUri, const std::string& strAddressTable, bool forceAck, bool initFec, 
                       bool scanFECs, bool scanCCUs,
		       tscType16 i2cSpeed, bool invertClockPolarity )
  throw (FecExceptionHandler ) {
  std::cerr << "FecAccess::FecAccess 7" << std::endl;
#if !defined(BUSUTCAFEC)
  RAISEFECEXCEPTIONHANDLER(CODECONSISTENCYERROR,
		    "Support for uTCA is not compiled and cannot be used",
		    FATALERRORCODE ) ;
  ...
  strBusAdapter_=strBoardId;
//Edit Georg Auzinger:
  FecUtcaRingDevice::configureUhal(strFilename, strBoardId, strUri, strAddressTable);
  fecBusType_ = FECUTCA ;
  // Initialise the object
  setInitFecAccess ( forceAck, initFec, scanFECs, scanCCUs,
		     i2cSpeed, 
...
//Edit G. Auzinger
    else if (strcasecmp(argv[i],"-fecUri") == 0) {
      if ((i+1) < argc) {
	i ++ ;
	if (argv[i][0] == '-') {
	  std::cerr << "value after -fecUri should not begin with '-'" << std::endl ;
	  return NULL ;
	}
	fecUri = std::string(argv[i]);
      }
      else {
	std::cerr << "Please specify the uTCA FEC URI string after option -fecuri" << std::endl ;
	return NULL ;
      }      
    } 
    else if (strcasecmp(argv[i],"-fecAddressTable") == 0) {
      if ((i+1) < argc) {
	i ++ ;
	if (argv[i][0] == '-') {
	  std::cerr << "value after -fecAddressTable should not begin with '-'" << std::endl ;
	  return NULL ;
	}
	fecAddressTable = std::string(argv[i]);
      }
      else {
	std::cerr << "Please specify the uTCA FEC AddressTable string after option -fecAddressTable" << std::endl ;
	return NULL ;
      }      
    } 
  }
...
//check if fecUri, fecAddressTable are empty strings and then decide accordingly
    if(fecUri.empty() && fecAddressTable.empty())
	fecAccess = new FecAccess(vmeFileName, fecHardwareId, forceAck, initFec, true, false, (tscType16)i2cSpeed, invertClockPolarity);
    else 
	fecAccess = new FecAccess(vmeFileName, fecHardwareId, fecUri, fecAddressTable, forceAck, initFec, true, false, (tscType16)i2cSpeed, invertClockPolarity);
    break;
  }//switch
...
FecAccess *FecAccess::createUtcaFecAccess (  const std::string& dummyfile, const std::string& fecHardwareId, const std::string& fecUri, const std::string& fecAddressTable, unsigned int i2cSpeed,  bool invertClockPolarity, bool init ) throw (FecExceptionHandler) {
  ...
  // FEC type
  //enumFecBusType fecBusType = FECUTCA  ;
  //enumBusAdapter busAdapter = CAENPCI ;

  std::string vmeFileName  = dummyfile;
